...
@section('content')
        ...
        <script type="text/javascript">
            $(document).ready(function () {
                ...
                dtTrns.on('click', '.showTrn', function (e) {
                    ...
                    let url = "{{ route('getTrnDetails', ':trnId') }}";
    ...
    <div class="card mb-3">
        <div class="bg-holder d-none d-lg-block bg-card"
             style="background-image:url('{{asset('media/illustrations/corner-4.png')}}');"></div>
        <!--/.bg-holder-->
        <div class="card-body">
            <div class="row">
                <div class="col-lg-8">
    ...
    <div class="row mt-3">
        <div class="col-md-12">
            <div class="card">
                <form action="{{ route('updateBulkTrnStatus') }}" method="POST" id="bulkUpdate">@csrf</form>
                ...
                <div class="card-body">
                    <div class="table-responsive fs--1">
                        <table id="dtTrns" class="table table-sm table-striped table-hover border-bottom">
                            ...
                            <tr>
                                ...
                                <th class="border-0 text-center" width="2%">Trn No.</th>
                                <th class="border-0 text-center" width="8%">Trn Date</th>
                                <th class="border-0 text-center" width="2%">Trn Type</th>
                                <th class="border-0 ">Payee</th>
                                <th class="border-0 ">Messer</th>
                                <th class="border-0 ">Description</th>
                                <th class="border-0 ">Trn Amount</th>
                                <th class="border-0 text-center" width="5%">Status</th>
                                <th class="border-0 text-center no-sort" width="2%">Action
                            </tr>
                            </thead>
                            ...
                            @forelse($trns as $trn)
                                <tr>
                                    ...
                                    <td class="text-center">{{ date('d-M-Y', strtotime($trn->trn_date)) }}</td>
                                    <td class="text-center">{{ $trn->trn_type }}</td>
                                    <td>{{ $trn->payee }}</td>
                                    <td>{{ $trn->messer }}</td>
                                    <td>{{ $trn->description }}</td>
                                    <td>{{ number_format($trn->trn_amount) }}</td>
                                    ...
                                    <td class="text-center">
                                        <div class="dropdown text-sans-serif position-static p-0">
                                            <button class="btn btn-sm btn-link dropdown-toggle btn-reveal"
                                                    type="button" id="dropdown5" data-toggle="dropdown"
                                                    data-boundary="html" aria-haspopup="true" aria-expanded="false">
                                                <span class="fas fa-ellipsis-h fs-1 py--1 px-2 border rounded border-400 bg-soft-info"></span>
                                            </button>
                                            <div class="dropdown-menu dropdown-menu-right border py-0"
                                                 aria-labelledby="dropdown5" style="">
                                                <div class="bg-white py-2">
                                                    <a class="dropdown-item showTrn" href="" data-id="{{ $trn->id }}">
                                                        View Transaction
                                                    </a>
{{--                                                    <form action="{{ route('getVoucherTrns') }}" method="POST">--}}
{{--                                                        @csrf--}}
{{--                                                        <input type="hidden" value="{{ $trn->id }}" name="trn_id">--}}
{{--                                                        <button type="submit" class="dropdown-item" href="#">--}}
{{--                                                            Print Transaction--}}
{{--                                                        </button>--}}
{{--                                                    </form>--}}
                                                    <div class="dropdown-divider"></div>
                                                    <form action="{{ route('updateTrnStatus', $trn->id) }}"
                                                          method="POST">
                                                        @csrf
                                                        @method('PUT')
