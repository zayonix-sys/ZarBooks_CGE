...
{{--        <script src="{{asset('lib/select2/select2.min.js')}}"></script>--}}
                ...
                $('#btnPrint').on('click', function (){
                    $('#dtBalanceSheet').printThis({
                        ...
                        header: '<h5 class="fs-2 py-2 py-xl-0 font-weight-bold text-center">' +
                            ...
                            'As On @isset($totalAndPeriod){{ $totalAndPeriod['to_date'] }}@endisset'+
                            '</span>' +
                            '</h5>',               // prefix to html
    ...
    <div class="row">
        <div class="col-md-3">
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('accounts/report/balanceSheet') }}" method="POST" id="frmCrEntry">
                        @csrf
                        <div class="form-row">
                        ...
                        <table id="dtBalanceSheet" class="table table-sm border"
                            ...
                            @isset($accounts)
                                        ...
                                        @foreach($account->account as $cntrlAccount)
                                            @if($cntrlAccount->balance != 0)
                                                <tr>
                                                    ...
                                                    <td class="text-right pr-3 ml-auto">{{ number_format($cntrlAccount->balance) }}</td>
                                                </tr>
                                            @endif
                                        @endforeach
                                    @endif
                                ...
                                <tr>
                                    ...
                                    <td class="text-right font-weight-bold pr-3 ml-auto bg-soft-success">Rs. {{ number_format($totalAndPeriod['totalAssets']) }}</td>
                                </tr>
                                {{-- Display Liabilites --}}
                                <tr class="bg-gray-800">
                                        ...
                                        @foreach($account->account as $cntrlAccount)
                                            @if($cntrlAccount->balance != 0)
                                                <tr>
                                                    ...
                                                    <td class="text-right pr-3">{{ number_format($cntrlAccount->balance) }}</td>
                                                </tr>
                                            @endif
                                        @endforeach
                                    @endif
                                ...
                                <tr>
                                    ...
                                    <td class="text-right font-weight-bold pr-3 bg-soft-warning">Rs. {{ number_format($totalAndPeriod['liabilities']) }}</td>
                                </tr>
                                {{-- Display Owner Equity --}}
                                <tr>
                                    <td class="text-left font-weight-bold text-underline">Owner Equities</td>
                                    <td></td>
                                </tr>
                                @forelse($accounts as $account)
                                    @if($account->account_group == 30)
                                        <tr>
                                            <td class="text-left font-weight-bold ">{{ $account->title }}</td>
                                            <td></td>
                                        </tr>
                                        @foreach($account->account as $cntrlAccount)
                                            <tr>
                                                ...
                                                <td class="text-right pr-3">{{ number_format(abs($cntrlAccount->balance)) }}</td>
                                            </tr>
                                        @endforeach
                                    @endif
                                @empty
                                ...
                                <tr>
                                    ...
                                    <td class="text-right pr-3">{{ number_format($totalAndPeriod['profitLoss']) }}</td>
                                </tr>
                                <tr>
                                    <td class="text-right font-weight-bold bg-soft-warning">
                                        Total Owner Equities <i class="fas fa-long-arrow-alt-right fs-2 pt-2"></i>
                                    </td>
                                    <td class="text-right font-weight-bold pr-3 bg-soft-warning">Rs. {{ number_format(abs($totalAndPeriod['totalOwnerEquity'])) }}</td>
                                </tr>
                                <tr>
                                    <td class="text-right font-weight-bold bg-soft-success">
                                        Total Equities <i class="fas fa-long-arrow-alt-right fs-2 pt-2"></i>
                                    </td>
                                    <td class="text-right font-weight-bold pr-3 bg-soft-success">Rs. {{number_format(abs($totalAndPeriod['totalEquities'])) }}</td>
                                </tr>
                            @endisset
                            </tbody>
                        </table>
