<?php if(session()->get('notify.model') === 'toast'): ?>
    <div class="notify fixed inset-0 flex items-end justify-center px-4 py-6 pointer-events-none sm:p-6 sm:items-start sm:justify-end">
        <div
            ...
            class="max-w-sm w-full <?php if(config('notify.theme') === 'light'): ?> bg-white <?php else: ?> bg-gray-800 <?php endif; ?> shadow-lg rounded-lg pointer-events-auto border-l-4 <?php if(session()->get('notify.type') === 'success'): ?> border-green-600 <?php endif; ?> <?php if(session()->get('notify.type') === 'warning'): ?> border-yellow-400 <?php endif; ?> <?php if(session()->get('notify.type') === 'info'): ?> border-blue-600 <?php endif; ?> <?php if(session()->get('notify.type') === 'error'): ?> border-red-600 <?php endif; ?>"
        >
                ...
                <div class="p-4">
                    <div class="flex items-start">
                        ...
                        <div class="ml-4 w-0 flex-1">
                            <p class="text-base leading-5 font-medium capitalize <?php if(session()->get('notify.type') === 'success'): ?> text-green-600 <?php endif; ?> <?php if(session()->get('notify.type') === 'warning'): ?> text-yellow-400 <?php endif; ?> <?php if(session()->get('notify.type') === 'info'): ?> text-blue-600 <?php endif; ?> <?php if(session()->get('notify.type') === 'error'): ?> text-red-600 <?php endif; ?>">
                                <?php echo e(session()->get('notify.title') ?? session()->get('notify.type')); ?>
...
<?php endif; ?>
<?php /**PATH I:\OneDrive\Zeeshan Programming\ZarBooks\vendor\mckenziearts\laravel-notify\src/../resources/views/notifications/toast.blade.php ENDPATH**/ ?>